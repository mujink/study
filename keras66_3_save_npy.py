...
xy_train = train_datagen.flow_from_directory(
    ...
    batch_size=160, # (전체 데이터/batch_size, batch_size,150,150,3 5차원 배열)
...
xy_test = test_datagen.flow_from_directory(
    ...
    batch_size=120, # (전체 데이터/batch_size, batch_size,150,150,3 5차원 배열)
                  # (만약 베치사이즈가 전체 데이터 사이즈보다 크면 4차원 배열 생성됨)
    # 폴더 안에 있는 이미지에 대해서 라벨값을 지정하는데
    # 라벨 값은 폴더 안에 있는 폴더의 이름 순서대로 지정하되
    # 바이너리 모드인 0 또는 1로 각각 지정한다.
...
np.save('../data/image/brain_npy/keras66_train_x.npy', arr=xy_train[0][0])
np.save('../data/image/brain_npy/keras66_train_y.npy', arr=xy_train[0][1])
np.save('../data/image/brain_npy/keras66_test_x.npy', arr=xy_test[0][0])
np.save('../data/image/brain_npy/keras66_test_y.npy', arr=xy_test[0][1])
