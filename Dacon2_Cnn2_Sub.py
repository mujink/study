...
for train_index, valid_index in skf.split(train2,train['digit']) :
    ...
    mc = ModelCheckpoint("../data/modelCheckPoint/Dacon2_Cnn2_"+str(nth+1)+".hdf5",save_best_only=True, verbose=1)
    ...
    model = load_model("../data/modelCheckPoint/Dacon2_Cnn2_"+str(nth+1)+".hdf5")
    ...
    result += model.predict_generator(test_generator,verbose=True)/100

    # save val_loss
    # hist = pd.DataFrame(learning_history.history)
    # val_loss_min.append(hist['val_loss'].min())/26
...
sub.to_csv('../data/csv/Dacon2/data/baseline_test2.csv', index=False)
