...
# codacdbset = pd.read_csv("../data/csv/KODEX 코스닥150 선물인버스.csv",encoding='cp949',index_col=0)
# datasets1 = pd.read_csv("../data/csv/삼성전자0115.csv",encoding='cp949',index_col=0)
...
# codacdbset = codacdbset.iloc[:1089,:]
...
# # 저장
# datasets.to_csv('../data/csv/sam_dc.csv', encoding='ms949', sep=",")
...
print(codacdbset.corr())
sns.set(font_scale=0.7)
sns.heatmap(data=codacdbset.corr(), square=True, annot=True, cbar=True)
plt.show()
...
y = [datasets.iloc[size-1:,3].values,
    datasets.iloc[size-2:,3].values] #(2378,)

x1_prd = datasets.iloc[-size-1:-1,:col].values
x2_prd = codacdbset.iloc[-size-1:-1,:col].values
# """
...
# train_test_split
x1 = split_x(datasets_minmaxed,size,col) # (2378,20,9)
x2 = split_x(codacdbset_minmaxed,size,col)

# 삼성 넘파이 저장
np.save('../data/npy/samsung_2.npy',arr=([x1,x2,y,x1_prd,x2_prd]))
x1 = np.load('../data/npy/samsung_2.npy',allow_pickle=True)[0]
x2 = np.load('../data/npy/samsung_2.npy',allow_pickle=True)[1]
y = np.load('../data/npy//samsung_2.npy',allow_pickle=True)[2]
x1_prd = np.load('../data/npy//samsung_2.npy',allow_pickle=True)[3]
x2_prd = np.load('../data/npy//samsung_2.npy',allow_pickle=True)[4]

x_prdEN1 = pd.DataFrame(x1_prd)
x_prdEN1['x_prdEN2'] = (x2_prd)
x_prdEN1.to_csv('../data/csv/x_prdEN1.csv', encoding='ms949', sep=",")

x1_train,x1_test,x2_train,x2_test,y_train,y_test = train_test_split(x1,x2, y,test_size=0.2, shuffle=True)
x1_train,x1_val,x2_train,x2_val,y_train,y_val = train_test_split(x1_train,x2_train,y_train,test_size=0.2 , shuffle=True)

x1_train=x1_train.reshape(-1,size,col).astype('float32')
x2_train=x2_train.reshape(-1,size,col).astype('float32')
x1_test=x1_test.reshape(-1,size,col).astype('float32')
x2_test=x2_test.reshape(-1,size,col).astype('float32')
x_val=x_val.reshape(-1,size,col).astype('float32')
x1_prd = x1_prd.reshape(-1,size,col).astype('float32')
x2_prd = x2_prd.reshape(-1,size,col).astype('float32')
